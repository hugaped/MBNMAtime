...
Another approach to investigate the time-course relationship is to perform several standard NMAs at different time "bins", time periods within which we are assuming treatment effects are constant over time. We can do this within a single plot using `binplot()` and specifying the boundaries of multiple time bins.
...
Results are plotted versus the network reference and are plotted on the relevant link scale (in this case identity - i.e. mean differences versus Pl_0 are shown). Each time bin window is marked on the plot by vertical dashed lines. The NMA estimates within each time bin are plotted as a horizontal solid black line with a shaded region indicating the 95% credible interval (prediction intervals can instead be plotted). The width of these shaded regions is equal to the range of time-points included in the NMA performed within that time-bin, which may therefore be more narrow than the time bin specified in the `binplot()` command.
