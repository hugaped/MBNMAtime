---
title: "MBNMAtime for Time-Course Model-Based (Network) Meta-Analysis (Version 0.2.0)"
author: "Hugo Pedder"
date: "`r Sys.Date()`"
...
* $\mu_i$ is the study reference effect (modelled as a nuisance parameter) in study $i$
* $\alpha_i$ is the response at time=0 in study $i$
* $\boldsymbol{d}_{t}$ is a vector of pooled relative effects for treatment $t$ whose length is the number of time-course parameters in the model
...
This can be useful to assess if the MBNMA predictions are in agreement with predictions from a lumped NMA model (within a specific range of time-points), and is a general indicator of the fit of the time-course model. However, it is important to note that the NMA model is not necessarily the "correct" model, since it ignores potential differences in treatment effects that may arise from lumping time-points together. The wider the range specified in `overlay.nma`, the greater the effect of lumping and the stronger the assumption of similarity between studies.
...
```{r, fig.height=3, results="hide", warning=FALSE}
