...
- `pd` argument in `mb.run()` has now been changed to `pD` and can only take the values
`TRUE` (the default) to calculate pD using the Kullback–Leibler divergence, or `FALSE`
to calculate pV (an approximation of pD).
